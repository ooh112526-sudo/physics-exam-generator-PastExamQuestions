...
# ... (依賴檢查與常數定義保持不變，請保留原有的 import 與常數) ...
# ==========================================
# 依賴套件與環境檢查
# ==========================================
HAS_GENAI = False
...
# ... (clean_json_string, crop_image, img_to_bytes 函式保持不變) ...
...
def parse_with_gemini(file_bytes, file_type, api_key, target_pages=None):
    ...
    for batch_idx, batch_imgs in enumerate(batches):
        # 實際頁碼 = 本次呼叫的起始頁 + 這一批的相對位置
        # 但因為我們現在設計為「parse_with_gemini」只處理一個單位，所以 batch_idx 其實只有 0
        
        prompt = f"""
        分析圖片中的高中物理試題。只擷取物理題。
        回傳 JSON List。包含 number, type(Single/Multi/Fill/Group), content, options, answer, chapter。
        座標 full_question_box_2d [ymin, 0, ymax, 1000]。
        page_index 為圖片在此批次中的索引(0開始)。
        """
