...
# [修正] 確保正確匯入 smart_importer
import smart_importer

# [修正] 移除不存在的 firebase_db 引用
# import firebase_db  <-- 這一行是導致錯誤的主因，已移除
...
class CloudManager:
    def __init__(self):
        ...
        try:
            ...
            # 策略 2：Streamlit Secrets (Streamlit Cloud 專用)
            try:
            ...
            # 策略 3：自動偵測 (Workload Identity / 本機 gcloud)
...
def process_single_file(filename, api_key, file_id_in_db=None):
    ...
    with st.spinner(f"正在分析 {filename}... (AI 思考中，請稍候)"):
        # 呼叫 smart_importer.parse_with_gemini
                    ...
                    if image_to_crop:
                        try:
                            if st_cropper:
                                ...
                                st_cropper(pil_ref, realtime_update=True, box_color='#FF0000', key=f"{selected_file}_cropper_{i}", aspect_ratio=None)
