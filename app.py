...
# ... (CloudManager 類別保持不變，直接複製上一版) ...
class CloudManager:
    # ... (程式碼省略，請使用上一版的完整內容，無需更動) ...
    def __init__(self):
...
# ... (Question 類別、Utility 函式等保持不變，直接複製上一版) ...
class Question:
...
def process_single_file(filename, api_key, file_id_in_db=None):
    ...
    with st.spinner(f"正在分析 {filename}... (這可能需要幾分鐘，請勿關閉視窗)"):
