...
LAST_UPDATED = "2026-02-02 16:20 (CST) [Feature: Re-auth, Sync-Page, Delete-Confirm, Group-Gen]"
...
# ==========================================
# UI Helpers: Sync Page Data (關鍵功能)
# ==========================================
def sync_page_data(page_items):
    """
    換頁前，將 widget 狀態寫回 session cache，避免資料遺失。
    """
            ...
            with st.expander(f"📄 {f['filename']} ({f.get('ai_status')})"):
                ...
                with c1:
                    ...
                    if status in ["已辨識", "部分失敗", "已匯入"]:
                        if st.button("🔄 再次辨識 (會清除舊紀錄)", key=f"reauth_{f['id']}"):
                            cloud_manager.reset_analysis_data(f['id']) # 重置狀態
                            st.rerun()
        ...
        if not all_res: st.warning("無題目資料")
                ...
                with st.container():
                    ...
                    with c_edit:
                        ...
                        if item['type'] == 'Group':
                            ...
                            is_group, g_start, g_end = smart_importer.check_is_group_header(item.get('content', ''))
                            if is_group: 
                                st.info(f"偵測到題組 (題號 {g_start}~{g_end})")
                                if st.button("轉為題組 / 自動產生子題", key=f"to_grp_{uik}"):
                                    item['type'] = 'Group'
                                    # 自動產生子題欄位
                                    subs = []
                                    count = (g_end - g_start) + 1
                                    if count > 0 and count < 10: # 合理範圍
                                        for k in range(count):
                                            subs.append({"content": "", "answer": "", "number": g_start + k})
                                    else:
                                        subs.append({"content": "", "answer": "", "number": 1})
                                    item['sub_questions'] = subs
                                    st.rerun()
                            else:
                                if st.button("轉為題組", key=f"man_grp_{uik}"):
                                    item['type'] = 'Group'
                                    item.setdefault('sub_questions', []).append({"content": "", "answer": "", "number": 1})
                                    st.rerun()
                    ...
                    with c_img:
                        ...
                        if mode == "AI 預截圖":
                            # [Feature 2.1] 顯示備份或目前圖片
                            curr_img = item.get('image_b64')
                            backup_img = item.get('ai_crop_backup_b64')
                            
                            # 若目前圖片跟備份不一樣，代表切過了，提供還原
                            if backup_img and curr_img != backup_img:
                                if st.button("還原為初始 AI 截圖", key=f"rst_img_{uik}"):
                                    item['image_b64'] = backup_img
                                    st.rerun()
                            
                            # 優先顯示目前的 (可能是手動切過的)
                            disp_img = curr_img or backup_img
                            if disp_img: st.image(base64.b64decode(disp_img), caption="目前圖片")
            ...
            if items_to_remove:
                for it in items_to_remove:
                    all_res.remove(it)
                # 若刪除後當頁沒東西，退回上一頁
                if st.session_state['review_page'] * PER_PAGE >= len(all_res) and st.session_state['review_page'] > 0:
                    st.session_state['review_page'] -= 1
                st.success("已刪除")
                st.rerun()

            if st.button("🚀 確認匯入題庫", type="primary", use_container_width=True):
                sync_page_data(current_items) # 最後同步
                ...
                for idx, item in enumerate(all_res):
                    ...
                    if item.get('use_image') and item.get('image_b64'):
                        try: final_img_data = base64.b64decode(item['image_b64'])
                        except: pass
