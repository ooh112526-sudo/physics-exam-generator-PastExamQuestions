    ...
    if not ready_files:
        ...
        with st.form(key=f"edit_form_{selected_file}_{current_page}"): # Key 加入頁碼避免衝突
            ...
            for i, cand in enumerate(current_batch_candidates):
                ...
                with c1:
                    cand.content = st.text_area(f"題目內容 #{i}", cand.content, height=100, key=f"{selected_file}_c_{real_idx}")
                    
                    if cand.q_type != "Group":
                        opts_text = "\n".join(cand.options)
                        new_opts = st.text_area(f"選項 #{i}", opts_text, height=80, key=f"{selected_file}_o_{real_idx}")
                        cand.options = new_opts.split('\n') if new_opts else []
                    
                    # 題型選擇 (中文)
                    current_type_zh = TYPE_MAP_EN_TO_ZH.get(cand.q_type, "單選")
                    new_type_zh = st.selectbox(f"題型 #{i}", TYPE_OPTIONS, index=TYPE_OPTIONS.index(current_type_zh), key=f"{selected_file}_t_{real_idx}")
                    ...
                    cand.predicted_chapter = st.selectbox(f"章節分類 #{i}", smart_importer.PHYSICS_CHAPTERS_LIST, index=chap_idx, key=f"{selected_file}_ch_{real_idx}")
                    ...
                    if image_to_crop:
                        try:
                            ...
                            if st_cropper:
                                ...
                                # 這裡僅供互動，實際裁切需要額外邏輯
                                st.caption("提示：調整紅框後，請確認下方的「暫存此頁修改」")
                            else:
                                st.error("元件載入失敗，顯示靜態圖片")
